...
TITLE: Move Set Member with Upstash Redis in TypeScript
DESCRIPTION: This TypeScript example demonstrates how to use the `smove` command with an Upstash Redis client. It illustrates adding initial members to a set, executing the `smove` operation, and shows the resulting state of both the source and destination sets.

SOURCE: https://upstash.com/docs/redis/sdks/ts/commands/set/smove.mdx
...
TITLE: Load Lua Script using Upstash Redis Client (TypeScript)
DESCRIPTION: This example demonstrates how to use the Upstash Redis client in TypeScript to load a Lua script into the Redis script cache. It shows the `scriptLoad` method call and how to define an inline Lua script.
...
TITLE: Upstash Pipelining API Response Syntax Example
DESCRIPTION: Shows the expected JSON array response format for the Upstash Redis pipelining API. Each item in the array corresponds to a command's result or error, maintaining the order of the original commands.
...
TITLE: Implement Redis Transactions (MULTI/EXEC) in Python
DESCRIPTION: This example demonstrates how to use Redis transactions, initiated with `multi()`, to ensure atomicity for a sequence of commands. All commands within a transaction are executed as a single, isolated operation. Similar to pipelines, commands are batched and executed with `.exec()`.
...
TITLE: API Setup: Implement Next.js API Route with Upstash Redis Counter
DESCRIPTION: Creates a Next.js API route at `/api/hello` that initializes an Upstash Redis client using the bound secrets. This route increments a 'counter' key in Redis on each request and returns the current count as a JSON response.
...
TITLE: Execute BITFIELD and BITFIELD_RO Commands in Python
DESCRIPTION: This snippet demonstrates how to use the `bitfield` and `bitfield_ro` methods in the `upstash-redis` Python client. These methods return chainable objects for building complex bitfield operations, which are then executed using the `.execute()` function. It covers both incrementing and getting bitfield values.
...
TITLE: Generate Laravel Todo Model with Resources
DESCRIPTION: Artisan command to create a new Todo Eloquent model along with its associated controller, factory, migration, and API resource files. This command streamlines the initial setup for a new database entity.
...
TITLE: Configure Serverless Function with AWS and Node.js
DESCRIPTION: This YAML configuration defines a serverless function named 'query' using AWS Lambda and Node.js 14.x. It sets up environment variables, HTTP API events for a GET request at '/query', and enables CORS.
...
TITLE: Efficient Redis Connection Management in Serverless JavaScript
DESCRIPTION: Provides a JavaScript example for a serverless function handler demonstrating how to initialize a Redis client, perform operations, and ensure the connection is properly closed to manage concurrent connections effectively.

SOURCE: https://upstash.com/docs/redis/troubleshooting/max_concurrent_connections.mdx
...
TITLE: Apply fixed-window rate limit for all routes using Authorization header (JSON)
DESCRIPTION: This configuration snippet demonstrates how to enable the Strapi Upstash Rate Limit plugin and apply a fixed-window rate limit across all routes ('*') for both GET and POST methods. The rate limit is identified by the 'Authorization' header, allowing 10 requests within a 20-second window.
...
TITLE: Apply fixed-window rate limit for all routes using IP address (JSON)
DESCRIPTION: This configuration snippet shows how to apply a fixed-window rate limit to all routes ('*') for GET and POST methods, using the client's IP address as the identifier source. It allows 10 requests within a 20-second window, providing a common way to limit access per user IP.
...
TITLE: Configure Serverless Project for Histogram API
DESCRIPTION: Serverless configuration (`serverless.yml`) defining the AWS provider, Node.js runtime, and environment variables, specifically `REDIS_URL`. It also sets up two Lambda functions, `record` and `get`, mapping them to HTTP API endpoints with CORS enabled.
...
TITLE: Execute Multiple Redis Commands via Upstash Pipelining
DESCRIPTION: Full `curl` command demonstrating how to send a batch of Redis commands (`SET`, `SETEX`, `INCR`, `ZADD`) to the Upstash `/pipeline` endpoint. This example combines the Redis commands into the JSON request body for a single HTTP request.
...
TITLE: Implement Serverless Histogram API Logic (handler.js)
DESCRIPTION: Node.js code for `handler.js` containing the core logic for the `get` and `record` Lambda functions. It handles API requests, interacts with Redis using `ioredis` to store and retrieve data, and uses `hdr-histogram-js` to build histograms from the retrieved values. A helper function `fixUrl` is included to correct Redis URL formats.
...
TITLE: Simple Node.js Express Application with Redis Counter
...
TITLE: Check Current Redis Keyspace Notification Configuration
DESCRIPTION: This command retrieves the current value of the `notify-keyspace-events` option using `CONFIG GET`. This allows you to verify which keyspace and keyevent notifications are currently enabled on the Redis instance, ensuring the desired configuration is active.

SOURCE: https://upstash.com/docs/redis/howto/keyspacenotifications.mdx
...
TITLE: Cache Data with Laravel Cache::remember
DESCRIPTION: Example of using Laravel's Cache::remember method. This method retrieves data from the cache by key; if not found, it executes the provided closure to fetch the data (e.g., all todos) and stores the result in the cache for a specified duration.
...
TITLE: Cloudflare Worker Leaderboard API Logic (`index.js`)
DESCRIPTION: Main JavaScript code for the Cloudflare Worker, handling incoming requests. It routes GET requests to `getLeaderboard` and POST requests to `addScore`, interacting with Upstash Redis and leveraging Cloudflare's edge caching.
...
TITLE: Test Laravel Todo API Index Endpoint
DESCRIPTION: Shell command to test the 'index' route of the Todo API using curl. This command sends an HTTP GET request to the /api/todos endpoint to verify that the API is functioning correctly and returning the list of todos.
...
TITLE: Next.js Pages Router Home Page with Upstash Redis Counter
DESCRIPTION: TypeScript/TSX code for the `/pages/index.tsx` file. This example demonstrates how to use `getServerSideProps` to fetch and increment a counter value from Upstash Redis, displaying it on the Next.js home page.
...
TITLE: Implement Stale-While-Revalidate with Cache::flexible
DESCRIPTION: Example of using Laravel's Cache::flexible method for stale-while-revalidate caching. It defines a "fresh" period (5 seconds) and a "stale" period (10 seconds), serving cached data immediately while asynchronously refreshing it during the stale period to balance performance and data freshness.
...
TITLE: Configure Strapi Upstash Rate Limit for All Routes with Authorization Header
DESCRIPTION: This JSON configuration enables the Strapi Upstash Rate Limit plugin and applies a fixed-window rate limit to all GET and POST routes. The rate limit identifies users based on the 'Authorization' header, allowing 10 tokens per 20-second window.

SOURCE: https://upstash.com/docs/redis/sdks/ratelimit-ts/integrations/strapi/configurations.mdx
...
TITLE: Increment Sorted Set Member Score (TypeScript)
DESCRIPTION: This TypeScript example demonstrates how to use the `redis.zincrby` method to increment the score of a member in a sorted set. It first adds a member with an initial score, then increments its score, and finally logs the updated score.
...
TITLE: Configure Strapi Upstash Rate Limit Using IP Address
DESCRIPTION: This JSON configuration enables the Strapi Upstash Rate Limit plugin and applies a fixed-window rate limit to all GET and POST routes. The rate limit identifies users by their IP address, allowing 10 tokens per 20-second window.
...
TITLE: Check Redis Script Existence in Python
DESCRIPTION: This Python example demonstrates how to use the `scriptExists` method with a Redis client to check for the presence of multiple scripts in the Redis cache using their SHA1 hashes. The method returns a list of booleans (represented as 1 or 0) indicating the existence of each script.

SOURCE: https://upstash.com/docs/redis/sdks/py/commands/scripts/script_exists.mdx
...
TITLE: Scan Sorted Set with ZSCAN in TypeScript
DESCRIPTION: Demonstrates how to use the Upstash Redis client in TypeScript to add elements to a sorted set and then scan it using `redis.sscan` (referring to ZSCAN functionality) to retrieve members matching a pattern. This example shows basic usage and result logging.

SOURCE: https://upstash.com/docs/redis/sdks/ts/commands/zset/zscan.mdx
...
TITLE: Calculate Set Difference with Upstash Redis (TypeScript)
DESCRIPTION: This TypeScript example demonstrates how to use the SDIFF command with Upstash Redis. It shows adding elements to two sets, then performing a difference operation to find elements present in the first set but not in the subsequent sets, and finally logging the result.
...
TITLE: Increment a hash field value using HINCRBY in TypeScript
DESCRIPTION: This example demonstrates how to use the `HINCRBY` command with Upstash Redis in TypeScript. It first sets an initial value for a hash field using `hset` and then increments it by a specified amount using `hincrby`, logging the new value.

SOURCE: https://upstash.com/docs/redis/sdks/ts/commands/hash/hincrby.mdx
...
TITLE: Flush All Keys in Redis Database (TypeScript)
DESCRIPTION: This snippet demonstrates how to use the `flushdb` command with the Upstash Redis client. It includes examples for both synchronous and asynchronous execution. The `flushdb` command permanently deletes all keys in the current database and should be used with extreme caution. The `async` parameter, when set to `true`, performs the operation without blocking the client.

SOURCE: https://upstash.com/docs/redis/sdks/ts/commands/server/flushdb.mdx
...
TITLE: Modify Redis Key Value with SETRANGE (TypeScript)
DESCRIPTION: Demonstrates how to use the `SETRANGE` command in Upstash Redis to modify a portion of a string value at a specified offset. It shows setting an initial value, then applying `SETRANGE`, and logging the new length. The example illustrates the effect on the key's value.

SOURCE: https://upstash.com/docs/redis/sdks/ts/commands/string/setrange.mdx
...
TITLE: Count Elements in Redis Sorted Set with ZCOUNT in Python
DESCRIPTION: This example demonstrates how to use the `zcount` method in Python to count elements within a Redis sorted set based on a score range. It first adds elements to a sorted set and then queries the count, showing how to use exclusive bounds.

SOURCE: https://upstash.com/docs/redis/sdks/py/commands/zset/zcount.mdx
...
TITLE: Fetch Weather Data from External API and Cache (Elixir)
DESCRIPTION: Defines `fetch_weather_from_api`, which makes an HTTP GET request to the WeatherAPI using an environment variable for the API key. Upon a successful 200 OK response, it decodes the JSON body, extracts relevant weather details, and then caches this information in Redis using `cache_weather_response` before returning the processed data.
...
TITLE: Apply Custom Rate Decrements with Upstash Ratelimit
DESCRIPTION: This example illustrates how to subtract a custom value from the available rate limit instead of the default '1'. This is particularly useful for scenarios where operations consume varying amounts of 'rate', such as processing user input in batches, allowing more granular control over rate limiting.
...
TITLE: Remove Single Random Member from Redis Set (Python SPOP)
DESCRIPTION: Demonstrates how to use the `SPOP` command in Python to remove and return a single random member from a Redis set. The example first adds 'one', 'two', and 'three' to 'myset' and then asserts that the popped member is one of these original values.
...
TITLE: Remove Multiple Random Members from Redis Set (Python SPOP with Count)
DESCRIPTION: Illustrates the usage of the `SPOP` command with the `count` argument in Python to remove and return multiple random members from a Redis set. The example adds 'one', 'two', and 'three' to 'myset' and then pops two members, asserting they are from the original set.
...
TITLE: Node.js ioredis ECONNRESET Error Stack Trace
DESCRIPTION: An example stack trace from a Node.js application using `ioredis` that shows the 'read ECONNRESET' error. This error typically indicates that the client's connection was unexpectedly reset by the server, often due to a mismatch in TLS configuration where the server expects a secure connection but the client attempts an insecure one.

SOURCE: https://upstash.com/docs/redis/troubleshooting/econn_reset.mdx
...
TITLE: SMISMEMBER Command API Reference
DESCRIPTION: API documentation for the Redis `SMISMEMBER` command, detailing its arguments and expected response.

SOURCE: https://upstash.com/docs/redis/sdks/py/commands/set/smismember.mdx
...
DESCRIPTION: Executes the AWS CDK commands to synthesize the CloudFormation template, bootstrap the AWS environment (if necessary), and deploy the defined AWS resources, including the Lambda function and its public URL.
...
TITLE: LPUSH Command API Reference
DESCRIPTION: Detailed API documentation for the Redis LPUSH command, outlining its required arguments (key and elements) and the integer response representing the new length of the list.
...
TITLE: API Documentation for Redis TYPE Command
DESCRIPTION: Comprehensive documentation for the Redis `TYPE` command, detailing its required arguments, expected response type, and the possible string values returned for different key types.
...
Arguments:
  - key: (type: str, required) The key to get.
...
TITLE: HEXISTS Redis Command API Documentation
DESCRIPTION: Detailed API documentation for the Redis `HEXISTS` command, outlining its required arguments, their types, and the expected boolean response.

SOURCE: https://upstash.com/docs/redis/sdks/py/commands/hash/hexists.mdx
...
HEXISTS Command:
  ...
  Response:
    Type: bool
    Required: true
    Description: 'True' if the hash contains 'field'. 'False' if the hash does not contain 'field', or 'key' does not exist.
```

--------------------------------

TITLE: Invoke Serverless Local Function for Producer Testing
DESCRIPTION: This shell command invokes a local serverless function named 'hello' with a JSON payload representing employee data. It's used to simulate a producer sending events to the consumer queue for testing purposes, allowing developers to verify the consumer's functionality.
...
TITLE: LLEN Command API Documentation
DESCRIPTION: Detailed API specification for the LLEN command, outlining its required arguments and the structure of its response.
...
TITLE: Deploy AWS CDK Application
DESCRIPTION: Executes the commands to synthesize, bootstrap, and deploy the AWS CDK stack. `cdk synth` generates CloudFormation templates, `cdk bootstrap` sets up necessary resources in the AWS account, and `cdk deploy` provisions the resources.
...
TITLE: INCRBY Command API Reference
DESCRIPTION: Detailed API documentation for the INCRBY command, including its arguments, return type, and behavior.
...
```
INCRBY Command:
  Description: Increment the integer value of a key by a given number. If a key does not exist, it is initialized as 0 before performing the operation. An error is returned if the key contains a value of the wrong type or contains a string that can not be represented as integer.
  Arguments:
    key:
      Type: string
      Required: true
  ...
  Response:
    Type: integer
    Required: true
    Description: The value at the key after the incrementing.
```

--------------------------------

TITLE: RPOP Command API Reference
DESCRIPTION: Comprehensive API documentation for the RPOP command, detailing its parameters, return types, and behavior for single and multiple element retrieval.
...
DESCRIPTION: Detailed API documentation for the MSET command, outlining its required parameters and expected response. This command counts as a single operation for billing.
...
MSET Command:
  ...
  Billing: Counts as a single command.
  Arguments:
    params: Record<string, TValue> (required)
      Description: An object where the keys are the keys to set, and the values are the values to set.
  Response:
...
TITLE: Define Python Dependencies for API
DESCRIPTION: Specifies the `upstash-redis` library as a required dependency for the Python serverless function. This file will be used by pip during the Lambda bundling process.
...
TITLE: Rate Limit Strategy Definition Parameters
DESCRIPTION: Defines an array of strategy objects, each specifying rate limits per route based on HTTP methods, path, identifier source, and the chosen rate limiting algorithm.
...
CODE:
```
methods: ('GET' | 'POST' | 'DELETE' | 'PUT' | 'PATCH' | 'ALL')[] (required) - An array of HTTP methods to apply the rate limit.
path: string (required) - The path to apply the rate limit.
identifierSource: string (required) - The source to identify the user for rate limiting.
debug: string - Enable debug mode for the route.
limiter: object (required) - The limiter configuration for the route.
  algorithm: 'fixed-window' | 'sliding-window' | 'token-bucket' (required) - The rate limit algorithm to use.
  tokens: number (required) - The number of tokens allowed in the time window.
  window: string (required) - The time window for the rate limit.
  refillRate: number - The rate at which the bucket refills (only for token-bucket algorithm).
```

--------------------------------

TITLE: Create FastAPI Application with Upstash Redis Counter
DESCRIPTION: Defines a basic FastAPI application in `main.py`. It initializes an Upstash Redis client using environment variables and creates a root endpoint that increments a counter in Redis on each access, returning the current count.
...
TITLE: Deploy AWS CDK Application
DESCRIPTION: These commands are used to deploy the AWS CDK application. `cdk synth` synthesizes the application into CloudFormation templates, `cdk bootstrap` prepares the AWS environment for CDK deployment (if not already done), and `cdk deploy` provisions the defined resources in your AWS account.
...
TITLE: HMSET Command API Reference
DESCRIPTION: Detailed API documentation for the HMSET command, including its arguments, types, and expected response.
...
HMSET:
  ...
  Arguments:
    key (str, required): The key of the hash.
    fields (Dict[str, Any], required): A dictionary of fields and their values.
  Returns:
    int (required): The number of fields that were added.
```
...
TITLE: Set Upstash Redis Environment Variables
DESCRIPTION: These bash commands export the `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` environment variables. These variables are crucial for the Node.js application to authenticate and connect to the Upstash Redis database.
...
TITLE: Redis SUNION Command API Reference
DESCRIPTION: Defines the arguments and response structure for the Redis SUNION command, including parameter types, requirements, and descriptions.
...
TITLE: XADD Command API Reference
DESCRIPTION: Detailed API specification for the XADD command, including all supported arguments, their types, descriptions, and the expected response format. This command appends new entries to a Redis stream.
...
XADD Command:
  Arguments:
    ...
    entries: Record<str, unknown> (required)
...
TITLE: Create API Directory for Lambda Function
DESCRIPTION: Creates a new directory named `api` inside the `lib` folder. This directory will house the Python code for the serverless Lambda function.
...
TITLE: ZDIFFSTORE Command API Reference
DESCRIPTION: Detailed API documentation for the Redis ZDIFFSTORE command, outlining its required arguments and the structure of its response.

SOURCE: https://upstash.com/docs/redis/sdks/ts/commands/zset/zdiffstore.mdx
...
ZDIFFSTORE Command:
  ...
  Arguments:
    destination:
      Type: string
      Required: true
      Description: The key to write the difference to.
    keys_count:
      Type: integer
      Required: true
      Description: How many keys to compare.
    keys_list:
      Type: ...string[]
      Required: true
      Description: The keys to compare.
...
TITLE: API Documentation for JSON.MERGE Command
DESCRIPTION: Comprehensive documentation for the `JSON.MERGE` command, detailing its required arguments (key, path, value) with their types and descriptions, and the expected boolean response upon successful execution.
...
JSON.MERGE:
  ...
  Arguments:
    ...
    value:
      Type: TValue
      Required: true
      Description: The value to merge with.
  Response:
    Type: true
    Required: true
    Description: Returns true if the merge was successful.
```

--------------------------------

TITLE: HSETNX Redis Command API Documentation
DESCRIPTION: Documents the HSETNX command for Redis, including its arguments (key, field, value) with their types and requirements, and the boolean return type indicating whether the field was set.
...
HSETNX:
  ...
  Response:
    Type: bool
    Required: true
    Description: `True` if the field was set, `False` if it already existed.
```

--------------------------------

TITLE: SMOVE Command API Reference
DESCRIPTION: Defines the arguments, their types, and descriptions required for the SMOVE command, along with the possible return values and their meanings.
...
SMOVE Command:
  Arguments:
    source: string (required)
      Description: The key of the set to move the member from.
    destination: string (required)
      Description: The key of the set to move the member to.
    member: TMember
      Description: The members to move
  Response:
    Type: 0 | 1 (required)
      Description: 1 if the member was moved, 0 if not.
```

--------------------------------

TITLE: Enable Auto-Pipelining for Upstash Redis Client
DESCRIPTION: This snippet demonstrates how to enable auto-pipelining when initializing the Upstash Redis client. It shows two common initialization methods: `Redis.fromEnv` for environment variable configuration and direct instantiation with a URL and token.
...
TITLE: OAuth 2.0 Code Exchange and Token Response (GUI App)
DESCRIPTION: This snippet illustrates the process for a GUI application to exchange an OAuth 2.0 authorization code for an access token and refresh token, followed by the expected JSON response. The exchange is performed against the Auth0 token endpoint, requiring client ID, authorization code, and redirect URI.
...
TITLE: JSON.RESP Command API Documentation
DESCRIPTION: Detailed API documentation for the Redis JSON.RESP command, including its arguments, their types, default values, and the structure of the response.
...
TITLE: Configure General Plugin Settings
DESCRIPTION: This section describes the general configuration options for the Upstash Ratelimit Strapi Plugin, primarily enabling or disabling its functionality.
...
TITLE: DECRBY Command API Reference
DESCRIPTION: Comprehensive documentation for the DECRBY command, detailing its behavior, required arguments, and the expected response type.
...
```
Command: DECRBY
Description: Decrement the integer value of a key by a given number. If a key does not exist, it is initialized as 0 before performing the operation. An error is returned if the key contains a value of the wrong type or contains a string that can not be represented as integer.
Arguments:
  key:
    type: str
    required: true
...
TITLE: Create New Supabase Function
DESCRIPTION: Initializes a new Supabase function named 'upstash-redis-counter' in the current project directory, setting up its basic structure for development.
...
TITLE: Configure Serverless.yml for Lambda Deployment
DESCRIPTION: Presents the YAML configuration for `serverless.yml`. This file defines the AWS Lambda service, its runtime, how environment variables for Redis credentials are passed, and the HTTP API endpoint for the `ratelimit` function.
...
TITLE: PUBLISH Command API Reference
DESCRIPTION: Defines the API for the Redis PUBLISH command, specifying its input parameters (channel and message) and the structure of its integer response.
...
PUBLISH Command:
  ...
  Response:
    Type: integer
    Required: true
    Description: The number of clients who received the message.
```

--------------------------------

TITLE: Approximate Sliding Window Rate Calculation
DESCRIPTION: Illustrates the approximation logic for determining if a request should be allowed in a sliding window algorithm. It shows how requests from the previous window are weighted and combined with current window requests to estimate the total rate.
...
TITLE: Upstash Ratelimit Client API Reference
DESCRIPTION: Comprehensive API documentation for the Upstash Ratelimit client, detailing its constructor options and the `limit` method's parameters and return values, specifically in the context of traffic protection and deny lists.

SOURCE: https://upstash.com/docs/redis/sdks/ratelimit-ts/traffic-protection.mdx
...
class Ratelimit {
  constructor(options: object)
    options: Configuration options for the Ratelimit client.
      redis: Redis - An instance of the Redis client (e.g., `Redis.fromEnv()`).
      limiter: RatelimitLimiter - The rate limiting algorithm to use (e.g., `Ratelimit.slidingWindow(count: number, duration: string)`).
      enableProtection: boolean = false - If true, enables deny list checking (manual and auto IP).
      analytics: boolean = false - If true, enables analytics tracking for requests.

  limit(identifier: string, options?: object): Promise<object>
    identifier: string - The primary identifier for rate limiting (e.g., "userId").
    options: object (optional) - Additional context for deny list checks.
      ip: string - An IP address to check against deny lists.
      userAgent: string - A user agent string to check against deny lists.
      country: string - A country code to check against deny lists.

    Returns: Promise<object> - An object containing the result of the limit operation.
      success: boolean - True if the request is allowed, false if denied.
      pending: Promise<void> - A promise that resolves when all asynchronous background operations (e.g., analytics, IP list updates) are complete.
      reason: string - The reason for denial if `success` is false (e.g., "denyList").
      deniedValue: string - The specific value (IP, user agent, country, or identifier) that caused the denial if `success` is false.
```

--------------------------------

TITLE: HSCAN Redis Command API Documentation
DESCRIPTION: Documents the HSCAN command for Redis, detailing its arguments, response format, and usage.
...
HSCAN Command:
  ...
  Arguments:
    key: str (required)
      Description: The key of the hash.
    cursor: int (required)
      Description: The cursor, use `0` in the beginning and then use the returned cursor for subsequent calls.
    match: str (optional)
      Description: Glob-style pattern to filter by field names.
    count: int (optional)
      Description: Number of fields to return per call.
  Response:
    Type: Tuple[number, List[str]] (required)
    Description: The new cursor and the fields. If the new cursor is `0` the iteration is complete.
```

--------------------------------

TITLE: SMOVE Command API Reference
DESCRIPTION: Detailed API documentation for the Redis SMOVE command, outlining its required and optional arguments, their types, and the expected boolean response.
...
SMOVE Command:
  ...
  Arguments:
    source (str, required): The key of the set to move the member from.
    destination (str, required): The key of the set to move the member to.
    member (str): The members to move

  Response:
    (bool, required): True if the member was moved, False if it was not.
```
...
TITLE: Add Redisson Maven Dependency
DESCRIPTION: Add the Redisson client library as a dependency in your `pom.xml` file to enable its use in your Java project. This dependency provides the necessary classes and methods for interacting with Redis via Redisson.
...
TITLE: LSET Command API Reference
DESCRIPTION: Documents the LSET command for Redis, detailing its required arguments (key, index, data) and the expected 'OK' response upon successful execution.
...
LSET Command:
  Arguments:
    ...
    index: number (required)
      Description: At which index to set the value.
    data: TValue (required)
      Description: The value to set.
  Response:
    OK: string (required)
      Value: `OK`
...
TITLE: MSET Command API Reference
DESCRIPTION: Documents the MSET command for Upstash Redis, including its arguments, expected response, and billing information.
...
TITLE: Listen to All Redis Keyspace Notification Channels
DESCRIPTION: This command allows you to subscribe to all keyspace and keyevent notification channels using `psubscribe`. This is useful for testing and observing all events published by the Redis instance after enabling notifications, providing a comprehensive view of activity.
...
TITLE: ZINCRBY Command API Documentation
DESCRIPTION: Detailed API specification for the ZINCRBY command, outlining its required arguments and the type of value returned upon successful execution.
...
TITLE: FLUSHALL Command API Reference
DESCRIPTION: Documents the FLUSHALL command for Upstash Redis, detailing its arguments and overall behavior.
...
```
FLUSHALL:
  description: Deletes all keys permanently. Use with caution!
  arguments:
    async:
      type: boolean
      description: Whether to perform the operation asynchronously. Defaults to synchronous.
```

--------------------------------

TITLE: ZMSCORE Command API Reference
DESCRIPTION: Detailed API documentation for the ZMSCORE command, outlining its arguments and expected response structure.
...
Response:
  members (List[str], required): The members of the sorted set.
```

--------------------------------

TITLE: ZMSCORE API Reference
DESCRIPTION: Detailed API documentation for the ZMSCORE command, outlining its required arguments and the structure of its response.
...
ZMSCORE:
  ...
  Response:
    members:
      type: TMember[]
      required: true
      description: The members of the sorted set.
```

--------------------------------

TITLE: Deploy AWS CDK Application
DESCRIPTION: Executes the necessary AWS CDK commands to synthesize the CloudFormation templates, bootstrap the AWS environment for CDK deployment (if not already done), and finally deploy the defined serverless resources to AWS.
...
TITLE: HEXPIRE Command API Reference
DESCRIPTION: Detailed API documentation for the `HEXPIRE` command, outlining its arguments, their types, requirements, and the structure of the response, including specific return values and their meanings.
...
HEXPIRE:
  ...
  Arguments:
    ...
    field:
      Type: string | number | (string | number)[]
      Required: true
      Description: The field or fields to set an expiration time for.
    seconds:
      Type: number
      Required: true
  ...
  Response:
    ...
    Description: A list of integers indicating whether the expiry was successfully set.
      ...
      - 1: if the expiration was successfully set.
      - 2: if called with 0 seconds/milliseconds or a past Unix time.
    See Also: https://redis.io/commands/hexpire
```

--------------------------------

TITLE: ZCOUNT Command API Reference
DESCRIPTION: Detailed API documentation for the ZCOUNT command, outlining its arguments, their types, requirements, and the structure of the response.
...
```
ZCOUNT Command:
  Description: Returns the number of elements in the sorted set stored at key filtered by score.
  Arguments:
    key:
      Type: string
      Required: true
    ...
    min:
      Type: integer | string
      Required: true
      Description: The minimum score to filter by. Use -inf to effectively ignore this filter. Use (number to exclude the value.
    max:
      Type: integer | string
      Required: true
      Description: The maximum score to filter by. Use +inf to effectively ignore this filter. Use (number to exclude the value.
  Response:
    Type: integer
    Required: true
    Description: The number of elements where score is between min and max.
```

--------------------------------

TITLE: PEXPIRE Command API Documentation
DESCRIPTION: Comprehensive API documentation for the Redis PEXPIRE command, detailing its arguments, their types, and the expected response values.
...
PEXPIRE Command:
  ...
  Arguments:
    ...
    - Name: milliseconds
      Type: integer
      Required: false
      Description: The number of milliseconds until the key expires.
  Response:
    Type: integer
    Description: '1' if the timeout was applied, '0' if 'key' does not exist.
```
...
TITLE: HSET Command API Reference
DESCRIPTION: Documents the HSET command for Redis, including its required arguments, expected return values, and their descriptions.
...
HSET:
  ...
  Arguments:
    key:
      ...
      Description: The key of the hash.
    fields:
      Type: { [fieldName]: TValue }
      Required: true
      Description: An object of fields and their values.
  Response:
    Type: integer
    Required: true
...
TITLE: Deploy the SST application
DESCRIPTION: Deploys the SST application, including the Next.js app and its serverless infrastructure, to AWS.
...
TITLE: Serverless Service Configuration
DESCRIPTION: A partial 'serverless.yml' configuration snippet defining the service name for the autocomplete API.
...
TITLE: Upstash Redis MGET API Reference
DESCRIPTION: Detailed API documentation for the MGET command in Upstash Redis. It specifies the required arguments (a list of keys) and the expected response format (a list of values, with `None` for non-existent keys). This command counts as a single operation for billing.
...
```
MGET Command:
  Description: Load multiple keys from Redis in one go. Counts as a single command for billing.
  Arguments:
    keys:
      Type: List[str]
      Required: true
      Description: Multiple keys to load from Redis.
  Response:
    Type: List[str]
    Required: true
    Description: An array of values corresponding to the keys passed in. If a key doesn't exist, the value will be None.
```

--------------------------------

TITLE: blockUntilReady() Method Behavior
DESCRIPTION: Notes that the `blockUntilReady()` method behaves identically to the `limit()` method in terms of command usage and functionality.

SOURCE: https://upstash.com/docs/redis/sdks/ratelimit-ts/costs.mdx

LANGUAGE: APIDOC
CODE:
```
blockUntilReady():
  Description: Works the same as limit().
```

--------------------------------

TITLE: INCR Command API Reference
DESCRIPTION: Detailed API documentation for the INCR command, including its purpose, behavior, required arguments, and expected response types.
...
Behavior: If a key does not exist, it is initialized as 0 before performing the operation. An error is returned if the key contains a value of the wrong type or contains a string that can not be represented as integer.
Arguments:
  - key:
    type: string
    required: true
...
TITLE: ZDIFFSTORE Command API Reference
DESCRIPTION: Detailed API documentation for the Redis ZDIFFSTORE command, including its required arguments (destination, keys) and the integer response indicating the number of elements in the resulting set.
...
ZDIFFSTORE:
  Arguments:
    destination (str, required): The key to write the difference to.
    keys (List[str], required): The keys to compare.
  Response:
...
TITLE: Implement Redis Pipelining in Python
DESCRIPTION: This snippet shows how to use Redis pipelining with the `upstash-redis` client to batch multiple commands. Pipelining reduces roundtrips to the server, improving performance. Commands are added to the pipeline and then executed using `.exec()`, returning a list of results in order.
...
TITLE: HSTRLEN Command API Reference
DESCRIPTION: Comprehensive API documentation for the HSTRLEN command, outlining its required parameters and the structure of its integer response.
...
HSTRLEN:
  ...
  Response:
    type: integer (required)
      Description: `0` if the hash or field does not exist. Otherwise the length of the string.
```

--------------------------------

TITLE: Define Rate Limiting Strategies per Route
DESCRIPTION: This section details how to define an array of strategies to apply different rate limits based on HTTP methods, specific paths (with wildcard support), and identifier sources (IP or header). It also includes options for debug logging and the core limiter configuration.
...
CODE:
```
methods: ('GET' | 'POST' | 'DELETE' | 'PUT' | 'PATCH' |'ALL')[] (required)
...
path: string (required)
  ...
  - `path: "/api/restaurants/:id"` 
  - `path: "/api/restaurants"`
identifierSource: string (required)
...
limiter: object (required)
  The limiter configuration for the route. The limiter object has the following
  properties:
  - algorithm: 'fixed-window' | 'sliding-window' | 'token-bucket' (required)
...
TITLE: API Documentation for RENAME Command
DESCRIPTION: Comprehensive documentation for the RENAME command, including its purpose, arguments (source and destination keys), and the boolean response indicating success. It also notes the behavior when renaming a nonexistent key.
...
```
RENAME Command:
  Description: Renames a key and overwrites the new key if it already exists. Throws an exception if the key does not exist.
  Arguments:
    source:
      Type: str
      Required: true
...
TITLE: SUNIONSTORE Command API Reference
DESCRIPTION: Defines the SUNIONSTORE command's arguments and their types, along with the structure and type of its response.
...
TITLE: Redis INCRBY Command API Documentation
DESCRIPTION: Detailed API documentation for the Redis INCRBY command, outlining its purpose, behavior with non-existent keys or wrong types, required arguments (key and increment), and the expected integer response.
...
INCRBY Command:
  ...
  Behavior:
    - If a key does not exist, it is initialized as 0 before performing the operation.
...
TITLE: SRANDMEMBER Redis Command API Reference
DESCRIPTION: Detailed API documentation for the Redis SRANDMEMBER command, including its arguments, types, and return values.
...
TITLE: Simple ZINTERSTORE Usage in Python
DESCRIPTION: Demonstrates basic usage of the `redis.zinterstore` command to find the intersection of two sorted sets and store the result in a new destination key.
...
TITLE: HINCRBY Command API Reference
DESCRIPTION: Detailed API documentation for the HINCRBY command, including its required arguments (key, field, increment) and the structure of its integer response.
...
TITLE: Access Koyeb Application Runtime Logs
DESCRIPTION: Retrieves and displays the live runtime logs for a deployed Koyeb service, useful for monitoring application behavior and debugging.
...
TITLE: Deploy Serverless Service to AWS
DESCRIPTION: Gives the command to deploy the configured Serverless service, including the Lambda function and API Gateway, to the AWS cloud environment.
...
TITLE: Upstash Redis Database Connection Parameters
DESCRIPTION: Configuration parameters for connecting the plugin to the Upstash Redis REST API, including authentication and key prefixing.
...
TITLE: Configure Strapi Upstash Rate Limit with Multiple Algorithms and Paths
DESCRIPTION: This JSON configuration demonstrates how to apply different rate limiting strategies to specific routes. It uses a fixed-window algorithm for '/api/restaurants/:id' and a token bucket algorithm for '/api/restaurants', both identified by the 'x-author' header.
...
TITLE: ZLEXCOUNT Redis Command API Documentation
DESCRIPTION: Detailed API documentation for the ZLEXCOUNT command, including its required arguments, their types and descriptions, and the expected response format.
...
ZLEXCOUNT:
  ...
  Arguments:
    ...
    min:
      Type: string
      Required: true
      Description: The lower lexicographical bound to filter by. Use - to disable the lower bound.
    max:
      Type: string
      Required: true
      Description: The upper lexicographical bound to filter by. Use + to disable the upper bound.
  Response:
    Type: integer
    Required: true
    Description: The number of matched.
```

--------------------------------

TITLE: ZRANGE Command
DESCRIPTION: No description
...
TITLE: SCAN Command API Reference
DESCRIPTION: Detailed API documentation for the SCAN command, including its parameters, their types, requirements, and a description of the returned data structure.
...
SCAN Command:
  ...
  Response:
    ...
    Required: true
    Description: The new cursor and the keys as a tuple. If the new cursor is 0 the iteration is complete. Use the new cursor for subsequent calls.
```

--------------------------------

TITLE: HRANDFIELD Command API Documentation
DESCRIPTION: Detailed API documentation for the HRANDFIELD command, including its parameters and expected response structure.
...
HRANDFIELD Command:
  ...
  Arguments:
    ...
    - Name: count
      Type: integer
      Required: false
      Description: Optionally return more than one field.
    - Name: withValues
      Type: boolean
      Required: false
      Description: Return the values of the fields as well.
  Response:
...
TITLE: Python Bitfield Set and Increment Operations
DESCRIPTION: Demonstrates how to use the `bitfield` command in Python to perform a combination of set and increment operations on a Redis bitfield. It initializes a key, then sets the first 4 bits and increments the next 4 bits, asserting the expected results.
...
TITLE: SETBIT Command API Reference
DESCRIPTION: Defines the arguments and response structure for the SETBIT command.
...
SETBIT Command:
  Arguments:
    key: string (required)
      Description: The key of the bitset
    offset: integer (required)
      Description: Specify the offset at which to set the bit.
    value: 0 | 1 (required)
...
TITLE: HINCRBYFLOAT Command API Reference
DESCRIPTION: Detailed API documentation for the Redis `HINCRBYFLOAT` command, outlining its required arguments and the type of value returned.
...
HINCRBYFLOAT:
  Arguments:
    key (str, required): The key of the hash.
    field (str, required): The field to increment
    increment (float, required): How much to increment the field by. Can be negative to subtract.
  Response:
    (float, required): The new value of the field after the increment.
```

--------------------------------

TITLE: Redis APPEND Command API Reference
DESCRIPTION: Detailed API documentation for the Redis APPEND command, including its arguments, types, and expected response.
...
Response:
  Type: integer
  Required: true
  Description: How many characters were added to the string.
```

--------------------------------

TITLE: Handle Home Page Request and Extract Weather Info (Elixir)
DESCRIPTION: Defines the `home` controller action, which processes user requests for weather information by calling `fetch_weather`. It then renders the appropriate response based on whether weather data was successfully retrieved or an error occurred. Also includes `get_weather_info`, a private helper function that pattern matches and extracts specific location and current weather details from the raw API response map.
...
TITLE: SINTER Command API Reference
DESCRIPTION: Documents the arguments and response structure for the Redis SINTER command.

SOURCE: https://upstash.com/docs/redis/sdks/ts/commands/set/sinter.mdx
...
TITLE: ZLEXCOUNT Command API Reference
DESCRIPTION: Detailed API documentation for the ZLEXCOUNT command, outlining its required arguments (key, min, max) and the expected integer response.
...
ZLEXCOUNT:
  Arguments:
    key (str, required): The key to get.
    min (str, required): The lower lexicographical bound to filter by. Use '-' to disable the lower bound.
    max (str, required): The upper lexicographical bound to filter by. Use '+' to disable the upper bound.
  Response:
    (int, required): The number of matched.
```

--------------------------------

TITLE: SMEMBERS Redis Command API Documentation
DESCRIPTION: Documents the `SMEMBERS` command, detailing its required arguments, expected response type, and overall purpose for retrieving all members of a Redis set.
...
TITLE: Configure Upstash Redis Environment Variables
DESCRIPTION: This snippet shows the structure of the `.env` file required to store Upstash Redis connection details. These variables are used by the application to establish a secure connection to the Redis instance.
...
TITLE: Configure Upstash Redis Environment Variables
DESCRIPTION: These commands set the `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` environment variables, which are required to connect to your Upstash Redis database. Replace placeholders with your actual credentials.
...
TITLE: Vercel Integration Environment Variables for Upstash KV
DESCRIPTION: This code block presents alternative environment variables specifically for projects utilizing the Vercel & Upstash integration. These variables, `KV_REST_API_URL` and `KV_REST_API_TOKEN`, simplify the connection process within the integrated environment.
...
TITLE: JSON.TYPE Command API Reference
DESCRIPTION: Comprehensive API documentation for the `JSON.TYPE` command, including its required arguments, optional parameters, and expected return types.
...
TITLE: Ratelimit SDK `limit()` Function Redis Command Costs
DESCRIPTION: Details the Redis command count and specific commands executed by the `limit()` function of the Ratelimit SDK for different algorithms (Fixed Window, Sliding Window, Token Bucket), considering cache results and algorithm states. This helps in understanding the cost implications of using the Ratelimit SDK.
...
Fixed Window Algorithm:
  Cache Result: Hit/Miss, Algorithm State: First, Command Count: 3, Commands: EVAL, INCR, PEXPIRE
  Cache Result: Hit/Miss, Algorithm State: Intermediate, Command Count: 2, Commands: EVAL, INCR
  Cache Result: Miss, Algorithm State: Rate-Limited, Command Count: 2, Commands: EVAL, INCR
  Cache Result: Hit, Algorithm State: Rate-Limited, Command Count: 0, Commands: _utilized cache_
```
...
Sliding Window Algorithm:
  Cache Result: Hit/Miss, Algorithm State: First, Command Count: 5, Commands: EVAL, GET, GET, INCR, PEXPIRE
  Cache Result: Hit/Miss, Algorithm State: Intermediate, Command Count: 4, Commands: EVAL, GET, GET, INCR
  Cache Result: Miss, Algorithm State: Rate-Limited, Command Count: 3, Commands: EVAL, GET, GET
  Cache Result: Hit, Algorithm State: Rate-Limited, Command Count: 0, Commands: _utilized cache_
```
...
Token Bucket Algorithm:
  Cache Result: Hit/Miss, Algorithm State: First/Intermediate, Command Count: 4, Commands: EVAL, HMGET, HSET, PEXPIRE
  Cache Result: Miss, Algorithm State: Rate-Limited, Command Count: 2, Commands: EVAL, HMGET
  Cache Result: Hit, Algorithm State: Rate-Limited, Command Count: 0, Commands: _utilized cache_
```

--------------------------------

TITLE: API Documentation for EVAL Command
DESCRIPTION: Comprehensive documentation for the `EVAL` command, detailing its purpose, required parameters (script, keys, arguments), and the structure of its return value when evaluating Lua scripts on the Redis server.
...
EVAL Command:
  ...
  Arguments:
    script:
      Type: string
      Required: true
      Description: The lua script to run.
    keys:
      Type: string[]
      Required: true
      Description: All of the keys accessed in the script.
...
TITLE: SSCAN Command API Reference
DESCRIPTION: Detailed API specification for the Redis SSCAN command. This section describes all available arguments, their types, whether they are required, and the structure of the command's response, including nested options.
...
SSCAN Command:
  ...
  Arguments:
    ...
    options:
      Type: Object
      Required: false
      Description: Optional parameters for scanning.
      Properties:
        match:
          Type: str
          Required: false
          Description: Glob-style pattern to filter by members.
        count:
          Type: number
          Required: false
          Description: Number of members to return per call.
  Response:
    Type: Tuple[number, TMember[]]
    Required: true
    Description: The new cursor and the members. If the new cursor is `0` the iteration is complete.
```
...
TITLE: Initialize Redis Database with Country Names
DESCRIPTION: JavaScript script ('initdb.js') to populate the Redis database with country names and their prefixes. It uses 'ioredis' to add terms to a sorted set, ensuring lexicographical order for autocomplete functionality.
...
TITLE: Verify QStash Messages with Receiver
DESCRIPTION: This snippet illustrates how to use the Upstash QStash Receiver to verify incoming messages. The receiver is initialized with current and next signing keys from environment variables. It shows how to call the `verify` method with the signature and body of a received message.
...
TITLE: HPEXPIRE Command API Reference
DESCRIPTION: Detailed API documentation for the HPEXPIRE command, outlining its parameters, their types, requirements, and the structure of the response.
...
HPEXPIRE:
  ...
  Arguments:
    ...
    fields:
      Type: Union[str, List[str]]
      Required: true
      Description: The field or list of fields within the hash to set the expiry for.
    milliseconds:
      Type: Union[int, datetime.timedelta]
      Required: true
      Description: The timeout in milliseconds as an integer or a `datetime.timedelta` object.
    nx:
      Type: bool
      Optional: true
      Description: Set expiry only when the field has no expiry. Defaults to `False`.
    xx:
      Type: bool
      Optional: true
      Description: Set expiry only when the field has an existing expiry. Defaults to `False`.
    gt:
      Type: bool
      Optional: true
      Description: Set expiry only when the new expiry is greater than the current one. Defaults to `False`.
    lt:
      Type: bool
      Optional: true
      Description: Set expiry only when the new expiry is less than the current one. Defaults to `False`.
  Response:
    Type: List[int]
    Required: true
    Description: A list of integers indicating whether the expiry was successfully set.
    Possible Values:
      ...
      1: if the expiration was successfully set.
      2: if called with 0 seconds/milliseconds or a past Unix time.
    See Also: https://redis.io/commands/hpexpire
```

--------------------------------

TITLE: Implement Multi-Region Ratelimiting with Upstash
DESCRIPTION: This code demonstrates how to set up `MultiRegionRatelimit` using multiple Redis instances, enabling low-latency ratelimiting for users across different geographical regions. It highlights the initialization with an array of Redis clients and the use of `slidingWindow` for the limiter, ensuring distributed state replication via CRDTs.
...
TITLE: Interact with Redis using Laravel Facade
DESCRIPTION: Demonstrates how to use Laravel's `Redis` Facade to perform basic operations like storing and retrieving data. This is useful for direct interactions with the Redis database for simple caching or temporary data storage.
...
DESCRIPTION: Illustrates how to set up the `.env` file with placeholders for `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN`. These environment variables are crucial for the Lambda function to connect to the Upstash Redis database.
...
TITLE: Upstash Redis Environment Variables Configuration
DESCRIPTION: This snippet illustrates how to configure the essential environment variables for connecting to an Upstash Redis database. It includes placeholders for the REST URL and Token, which are required for authentication and API access.
...
TITLE: Redis ECHO Command API Documentation
DESCRIPTION: Detailed API specification for the Redis ECHO command, outlining its parameters and expected return value.
...
TITLE: HSTRLEN Command API Reference
DESCRIPTION: Comprehensive documentation for the HSTRLEN command, outlining its required parameters (key and field) and the integer response indicating the string length or 0 if the hash/field does not exist.
...
TITLE: Chain Commands in Redis Pipelines in Python
DESCRIPTION: This snippet illustrates a more concise way to add multiple commands to a Redis pipeline by chaining method calls. This syntax can improve readability for sequences of operations within a pipeline.
...
TITLE: Update `wrangler.toml` for Deployment
DESCRIPTION: Modification to the `wrangler.toml` file, changing the worker type to `webpack` for deployment purposes.
...
DESCRIPTION: Detailed API documentation for the SREM command, including its required arguments and the structure of its response.
...
TITLE: JSON.ARRAPPEND Command API Reference
DESCRIPTION: Detailed API documentation for the `JSON.ARRAPPEND` command, outlining its parameters, their types, descriptions, and the expected response format when appending values to a JSON array in Redis.
...
JSON.ARRAPPEND:
  ...
  response:
    type: List[int]
    required: true
    description: The length of the array after the appending.
```

--------------------------------

TITLE: ZRANDMEMBER Redis Command API Specification
DESCRIPTION: Detailed API specification for the ZRANDMEMBER command, outlining its required and optional arguments, their types, and the structure of the command's response.
...
ZRANDMEMBER Command:
  ...
  Arguments:
    ...
    withscores:
      Type: bool
      Required: false
      Description: Whether to return the scores along with the members
  Response:
    Type: str | Tuple[str, float] | List[str] | List[Tuple[str, float]]
    Description: The random member(s) from the sorted set. If no count is specified, a single member is returned. If count is specified, a list of members is returned. If withscores, members are returned as a tuple of (member, score).
```
```
