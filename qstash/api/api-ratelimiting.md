...
When a rate limit is exceeded, the API returns a 429 status code along with specific headers that provide information about the limit, remaining requests/tokens, and reset time. You can learn more about QStash plans and their limits on the QStash pricing page.
### 
​
Daily Rate Limit
This is a **daily** limit applied to publish-related API endpoints (new message requests like publish, enqueue, or batch). Other API requests, such as fetching logs or messages, do not count toward this limit. **Headers** :
...
​
Burst Rate Limit
This is a short-term limit **per second** to prevent rapid bursts of requests. This limit applies to all API endpoints. **Headers** :
...
This limit is applied to chat-related API endpoints. **Headers** :
...
Create Chat CompletionOverview
Assistant
