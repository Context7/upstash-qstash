...
We may truncate the response body if it exceeds your plan limits. You can check your `Max Message Size` in the console. Make sure you verify the authenticity of the callback request made to your API by verifying the signature.
# 
​
What is a Failure-Callback?
Failure callbacks are similar to callbacks but they are called only when all the retries are exhausted and still the message can not be delivered to the given endpoint. This is designed to be an serverless alternative to List messages to DLQ. You can add a failure callback URL in the `Upstash-Failure-Callback` header when publishing a message. The value must be a valid URL.
cURL
Typescript
Python
Copy
