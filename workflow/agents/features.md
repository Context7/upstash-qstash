...
The parameters for defining an agent are as follows:
  ...
  * **`name`**: The name of the agent. This name will be used when naming thecontext.call steps for this agent. `context.call` is used when calling the LLM provider (such as OpenAI).
...
Getting StartedPrompt Chaining
Assistant
